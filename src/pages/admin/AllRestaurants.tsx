...
const getDisplayPlan = (plan?: string) => {
  ...
  return mappedPlan
    ? APP_CONFIG.plans[mappedPlan as keyof typeof APP_CONFIG.plans]
    : undefined;
};
...
const AllRestaurants: React.FC = () => {
  ...
  const getPlanBadge = (plan: string) => {
    const planConfig = getDisplayPlan(plan);
    if (!planConfig) return null;

    return (
      <Badge
        variant={plan === "starter" || plan === "restaurant_plan" ? "success" : "neutral"}
...
const DetailsModal: React.FC<DetailsModalProps> = ({
  ...
  const planConfig = getDisplayPlan(restaurant.subscription_plan);
          ...
          <div className="bg-bg-subtle rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <p className="text-text-secondary">
                ...
                {planConfig.price.toLocaleString("en-IN")}
