...
export const createRestaurantAccount = async (
  ...
  try {
    ...
    const dbSubscriptionPlan =
      data.subscriptionPlan === "restaurant_plan"
        ? "starter"
    ...
    if (!result || result.length === 0 || !result[0].success) {
      throw new Error(
        getCreateAccountErrorMessage(
          result?.[0]?.message || "Failed to create restaurant"
        )
      );
    }
...
const getCreateAccountErrorMessage = (message = "") => {
  const normalized = message.toLowerCase();

  if (
    normalized.includes("restaurants_email_key") ||
    normalized.includes("users_email_key") ||
    normalized.includes("already used") ||
    normalized.includes("duplicate key")
  ) {
    return "This email is already used by an existing restaurant account. Use a different email address or manage the existing restaurant.";
  }

  if (normalized.includes("registration request has already been approved")) {
